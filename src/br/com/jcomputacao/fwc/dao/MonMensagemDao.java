...
import br.com.jcomputacao.fwc.model.MonMensagem;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;

public class MonMensagemDao {

	public String getSelect() {
		return "SELECT M.COD_MENSAGEM,M.DAT_RECEBIMENTO,M.NUM_LINHA,M.COD_CLIENTE,M.NOM_HOST,M.NUM_DBID,M.TIP_MENSAGEM,M.DAT_ENVIO,M.VAL_LINHA FROM MON_MENSAGEM M";
	}

	public String getInsert() {
		return "INSERT INTO MON_MENSAGEM (COD_MENSAGEM,DAT_RECEBIMENTO,NUM_LINHA,COD_CLIENTE,NOM_HOST,NUM_DBID,TIP_MENSAGEM,DAT_ENVIO,VAL_LINHA) VALUES (?,?,?,?,?,?,?,?,?)";
	}

	public String getUpdate() {
		return "UPDATE MON_MENSAGEM SET COD_MENSAGEM=?,DAT_RECEBIMENTO=?,NUM_LINHA=?,COD_CLIENTE=?,NOM_HOST=?,NUM_DBID=?,TIP_MENSAGEM=?,DAT_ENVIO=?,VAL_LINHA=?";
	}

	public String getDelete() {
		return "DELETE FROM MON_MENSAGEM WHERE ID=?";
	}


	protected void prepareUpdate(PreparedStatement stmt, MonMensagem m) throws SQLException {
		int idx = 1;
	...
	protected void setValues(MonMensagem m, ResultSet rs) throws SQLException {
